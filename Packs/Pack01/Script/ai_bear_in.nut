...
//┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏
{
	...
	// 逃げるときの索敵範囲
	const of32EscapeArea = 5.0;
	
	// 逃げるときの速度倍率とモーションスピード
	const of32EscapeSpeed = 3.0;
	const of32EscapeMotionSpeed = 1.0;
	
	// 突貫１の速度倍率とモーションスピード
	const of32WalkSpeed = 3.0;
	const of32WalkMotionSpeed = 2.5;
	
	// 突貫２の速度倍率とモーションスピード
	const of32RunSpeed = 3.0;
	const of32RunMotionSpeed = 1.2;
	...
	switch(pcOwn.getState()) {
		...
		case Enum.STATE_EX0:
			...
			if (!pcOwn.isMaxRangeIn(pcPlayer.getPosition())) {
				...
				// 走りモーション入り
				pcOwn.setMotionTag(MOTION_B_RUN, 0, 0.5, of32EscapeMotionSpeed, false, false);
		...
		case Enum.STATE_EX1:
			if (pcOwn.isEndMotin()) {
				...
				// 走りモーション
				pcOwn.setMotionTag(MOTION_B_RUN, 1, 0.3, of32EscapeMotionSpeed, true, false);
		...
		case Enum.STATE_EX3:
		{
			...
			// 走りモーション戻り
			pcOwn.setMotionTag(MOTION_B_RUN, 2, 0.3, of32EscapeMotionSpeed, false, false);
		...
		case Enum.STATE_EX9:
			if (pcOwn.isEndMotin()) {
				...
				// 歩きモーション入り
				pcOwn.setMotionTag(MOTION_WALK, 0, 0.5, of32WalkMotionSpeed, false, false);
		...
		case Enum.STATE_EX10:
			if (pcOwn.isEndMotin()) {
				...
				// モーションを歩きに
				pcOwn.setMotionTag(MOTION_WALK, 1, 0.3, of32WalkMotionSpeed, true, false);
		...
		case Enum.STATE_EX12:
			...
			// 歩きモーション戻り
			//pcOwn.setMotionTag(MOTION_WALK, 2, 0.3, of32WalkMotionSpeed, false, false);
		...
		case Enum.STATE_EX18:
			...
			// 走りモーション戻り
			//pcOwn.setMotionTag(MOTION_B_RUN, 2, 0.3, of32RunMotionSpeed, false, false);
