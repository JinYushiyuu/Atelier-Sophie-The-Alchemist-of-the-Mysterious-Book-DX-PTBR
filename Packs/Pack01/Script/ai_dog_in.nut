...
//┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏
{
	...
	local as32StatePattern = [ 
		Enum.STATE_PATTERN0, Enum.STATE_PATTERN2, Enum.STATE_PATTERN2, Enum.STATE_PATTERN2, Enum.STATE_PATTERN2, Enum.STATE_PATTERN2,
	]; // 逃げる、突貫１、突貫２、シカト
	...
	const of32EscapeMotionSpeed = 2.2;
	
	// 突貫１の速度倍率とモーションスピード
	const of32WalkSpeed = 1.0;
	const of32WalkMotionSpeed = 2.2;
	
	// 突貫２の速度倍率とモーションスピード
	const of32RunSpeed = 1.2;
	const of32RunMotionSpeed = 2.2;
	...
	switch(pcOwn.getState()) {
		...
		case Enum.STATE_EX0:
			...
			if (!pcOwn.isMaxRangeIn(pcPlayer.getPosition())) {
				...
				// 走りモーション入り
				pcOwn.setMotionTag(MOTION_B_RUN, 0, Val.MOTION_BLEND_FRAME, of32EscapeMotionSpeed, false, false);
		...
		case Enum.STATE_EX9:
			if (pcOwn.isEndMotin()) {
				...
				// 歩きモーション入り
				pcOwn.setMotionTag(MOTION_WALK, 0, Val.MOTION_BLEND_FRAME, of32WalkMotionSpeed, false, false);
		...
		case Enum.STATE_EX10:
			if (pcOwn.isEndMotin()) {
				...
				// モーションを歩きに
				pcOwn.setMotionTag(MOTION_WALK, 1, Val.MOTION_BLEND_FRAME, of32WalkMotionSpeed, true, false);
