	...
	switch(pcOwn.getState()) {
		...
		case Enum.STATE_MOVE_IN:
			...
			if (pcOwn.isMovePosition()) {
				...
				// 入りモーション
				pcOwn.setMotionTag(MOTION_B_RUN, 0, Val.MOTION_BLEND_FRAME, Val.MOTION_SPEED, false, false);
		...
		case Enum.STATE_MOVE_OUT:
			...
			// 戻りモーション
			pcOwn.setMotionTag(MOTION_B_RUN, 2, Val.MOTION_BLEND_FRAME, Val.MOTION_SPEED, false, false);
