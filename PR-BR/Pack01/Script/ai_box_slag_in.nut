...
//┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏
{
	...
	local as32StatePattern = [ 
		Enum.STATE_PATTERN1, Enum.STATE_PATTERN3,
	]; // 逃げる、突貫１、シカト
	...
	// 突貫１の速度倍率とモーションスピード
	const of32WalkSpeed = 2.0;
	const of32WalkMotionSpeed = 1.5;
	
	// 突貫２の速度倍率とモーションスピード
	const of32RunSpeed = 2.0;
	const of32RunMotionSpeed = 1.5;
	...
	switch(pcOwn.getState()) {
		...
		case Enum.STATE_WAIT:
			...
			// 待機モーションの発行
			pcOwn.setMotionTag(MOTION_F_WAIT, 0, 0.45, Val.MOTION_SPEED, true, true);
		...
		case Enum.STATE_EX20:
		{
			...
			pcOwn.setMotionTag(MOTION_EX9, 0, Val.MOTION_BLEND_FRAME, 0.65, false, false);
		...
		case Enum.STATE_EX21:
			if (pcOwn.isEndMotin()) {
				//pcOwn.setMotionTag(MOTION_EX9, 1, Val.MOTION_BLEND_FRAME, Val.MOTION_SPEED, false, false);
