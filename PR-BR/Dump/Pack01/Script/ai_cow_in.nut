...
//┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏┏
{
	...
	local as32StatePattern = [ 
		Enum.STATE_PATTERN1, Enum.STATE_PATTERN2, Enum.STATE_PATTERN3,
	]; // 逃げる、突貫１、突貫２、シカト
	
	// 待機時間
	const of32WaitTime = 0.7;
	...
	switch(pcOwn.getState()) {
		...
		case Enum.STATE_WAIT:
			...
			// 待機モーションの発行
			pcOwn.setMotionTag(MOTION_F_WAIT, 0, 0.2, Val.MOTION_SPEED, true, false);
		...
		case Enum.STATE_EX7:
		{
			...
			pcOwn.setMotionTag(MOTION_EX8, 0, 0.2, 2.0, false, false);
		...
		case Enum.STATE_EX9:
			if (pcOwn.isEndMotin()) {
				...
				// 歩きモーション入り
				pcOwn.setMotionTag(MOTION_WALK, 0, 0.2, of32WalkMotionSpeed, false, false);
		...
		case Enum.STATE_EX10:
			if (pcOwn.isEndMotin()) {
				...
				// モーションを歩きに
				pcOwn.setMotionTag(MOTION_WALK, 1, 0.2, of32WalkMotionSpeed, true, false);
		...
		case Enum.STATE_EX14:
		{
			...
			pcOwn.setMotionTag(MOTION_EX8, 0, 0.2, 1.0, false, false);
		...
		case Enum.STATE_EX15:
			if (pcOwn.isEndMotin()) {
				...
				// 走りモーション入り
				pcOwn.setMotionTag(MOTION_RUN, 0, 0.2, of32RunMotionSpeed, false, false);
		...
		case Enum.STATE_EX16:
			if (pcOwn.isEndMotin()) {
				...
				// モーションを走りに
				pcOwn.setMotionTag(MOTION_RUN, 1, 0.2, of32RunMotionSpeed, true, false);
		...
		case Enum.STATE_EX18:
			...
			// 走りモーション戻り
			//pcOwn.setMotionTag(MOTION_RUN, 2, 0.3, of32RunMotionSpeed, false, false);
